...
public:
    MainWindow(juce::String name) : juce::DocumentWindow(name,
        ...
        // macOS에서 더 강력한 포커스 설정
        #if JUCE_MAC
        setAlwaysOnTop(true);
        setAlwaysOnTop(false);
        
        // NSWindow 직접 조작으로 독립적인 앱 강제
        juce::MessageManager::callAsync([this]() {
            if (auto* peer = getPeer()) {
                if (auto* window = peer->getWindowHandle()) {
                    if (auto* nsWindow = (NSWindow*)window) {
...
public:
    ...
    void initialise(const juce::String&) override {
        // macOS에서 독립적인 앱으로 강제 설정
        #if JUCE_MAC
        juce::Process::setDockIconVisible(true);
        // 앱을 활성화하고 포커스 강제
        juce::Desktop::getInstance().getMainMouseSource().forceMouseCursorUpdate();
        
        // NSWindow 직접 생성으로 독립적인 앱 강제
        if (auto* peer = juce::ComponentPeer::getPeerUnderMouse()) {
            if (auto* window = peer->getWindowHandle()) {
                // NSWindow를 독립적인 앱으로 설정
